...
public class EmpDaoImpl implements EmpDao {
	...
	// 테이블 로우의 칼럼들을 도메인 클래스 멤버변수들에 어떻게 옮기는가를 정의한 로직
	private RowMapper<Emp> rowMapper = (ResultSet rs, int rowNum) -> {
		// ResultSet: 쿼리의 결과를 보관하는 객체
		Emp e = new Emp();
	...
	public int insert(Emp emp) {
		...
		int affected = jdbcTemplate.update(sql,	emp.getEmpno(), emp.getEname(), emp.getJob(), emp.getSal());
		return affected;
...
//		throw new RuntimeException("트랜잭션 테스트 용 언체크드 예외");
	...
	public int update(Emp emp) {
		...
		return jdbcTemplate.update(sql, emp.getEname(), emp.getJob(), emp.getSal(), emp.getEmpno());
	}
