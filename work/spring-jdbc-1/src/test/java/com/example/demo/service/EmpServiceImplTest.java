...
public class EmpServiceImplTest {
	...
	public void testInsert() {
		...
		assertEquals("insert 메소드에서 예외발생, 트랜잭션 어드바이스 적용, " + "롤백이 되어야 하기 때문에 " + "oldCount 값과 nowCount 값은 같아야 한다.",
				oldCount, nowCount);
	}

	// 다음 2개의 애노테이션을 동시에 설정하면 메소드가 종료된 후 메소드 내에서 행한 모든 쿼리작업이 롤백됩니다.
	@Transactional
	@Test
...
//		}
	...
	public void testDelete() {
		...
		try {
			int insertAffected = empService.count();
			System.out.println("insertAfterCount = " + insertAffected);
		} catch (Exception ignore) {
		}
