...
//			produces = { "text/plain" }, // 결과의 MIME 타입
			consumes = { // 클라이언트로부터 받는 자료의 MIME 타입
					"text/plain", 
					"application/*", 
					"text/html" })
	...
	// 클라이언트의 지역정보가 담겨 있다.
	@GetMapping("/example2")
	public String example2(HttpServletRequest req, Model model, HttpSession session, Locale locale) {
	...
	public ModelAndView example3(
			...
			User user) { // 파라미터를 받는 방법 #2, 사용방법: 파라미터 키 == 멤버변수명
	...
	public String example4(HttpServletRequest req, 
			@RequestParam(required = true) String id, // 파라미터를 한개만 받고싶을때, 생략가능(파라미터 변수명이 키값이됨)
...
//		"/" 문자열을 잘라서 배열로 전환
		...
		return req.getRequestURI() + ", " + id + ", " + age;
	}
