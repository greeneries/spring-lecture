...
public class ExampleController2 {
	...
	/*
	 *	사용자에게 많은 데이터를 받아야 하는 경우 한 화면에서 이를 모두 요구하면 사용자가 지치므로 
	 *  데이터 작성을 몇 단계로 나누어서 처리하고 싶을 때가 있다.
	 *  예: 선수 등록 Wizard
	 */
	...
	public String step3(
			...
			BindingResult result, // 사용자가 보낸 데이터와 관련된 에러 데이터
			HttpSession session,
		...
		// redirect: DS가 처리하라는 문법(재접속)
	}

	@RequestMapping("/step4")
	public String step4() {
		// 선수 등록 완료 안내 페이지
		// 처리결과는 FlashAttribute 객체에 추가한 정보가 자동으로 뷰에게 전달됩니다.
		// 뷰 처리가 끝나서 다시 DS에게 리턴될 때 FlashAttribute 객체는 파괴됩니다.
		return "step4";
	}
}

// 브라우저가 PUT, POST 하면 데이터를 넣고 리다이렉트로 돌려보낸다음 다시 GET방식으로 데이터를 얻고 성공을 돌려보낸다.