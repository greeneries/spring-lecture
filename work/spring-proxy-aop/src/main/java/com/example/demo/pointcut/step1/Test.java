...
public class Test {
	public static void main(String[] args) {
		 ...
		 * * one*(..) : 리턴자료형, 메소드명, (파라미터) 
		 * * : all : 리턴자료형이 무엇이든 상관없다. void, int, Object ...
...
//		pointcut.setExpression("execution(* one*(..))");
		...
		// int형의 파라미터 2개를 받는 메소드에 어드바이스를 적용하고싶다.
//		pointcut.setExpression("execution(* *(int, int))");

		// 첫번째 파라미터로 int형의 값을 받는 메소드에 어드바이스를 적용하고싶다.
//		pointcut.setExpression("execution(* *(int, ..))");
		
		// 파라미터를 2개 이상 받는 메소드 
//		pointcut.setExpression("execution(* *(*, *, ..))");
		
		// Second 클래스이고 메소드명에 알파벳 o가 있는 메소드
//		pointcut.setExpression("execution(* com.example.demo.pointcut.step1.Second.*o*(..))");
				
		pointcut.setExpression("execution(* com.example.demo.pointcut.step1..s*.*(..))");
